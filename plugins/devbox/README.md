...
一款集成在 ZTools 中的实用工具箱，提供随机身份、密码、数字、UUID、颜色、加密签名、中文转拼音、HTML预览、时间转换等多种生成工具。
...
### 时间转换
**触发指令**: `时间` / `时间转换` / `time` / `timestamp` / `时间戳`

显示当前时间的多种格式，支持输入时间戳或日期字符串进行格式转换。

**功能**:
- 实时时钟显示
- 11 种时间格式：本地时间、ISO 8601、UTC、RFC 2822、Unix 时间戳、相对时间等
- 输入识别：支持 Unix 时间戳（秒/毫秒）、ISO 日期字符串
- 时区切换：本地、UTC、北京、东京、纽约等常用时区
- 点击任意格式行即可复制
